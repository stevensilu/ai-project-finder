...
会话中输入的每一条需求都会进入索引，包括长对话里最近的那一条。单条超过 50,000
字符的需求（例如粘贴的大段文件）会被截断到该长度，刷新时会报告受影响的会话。AI
的回复不进入索引。
...
生成的索引包含每一条需求的完整文本，以及文件名和工作目录，因此会随会话数量增长而明显变大。分享已安装的应用副本前，建议将 `data/` 目录视为私有数据并进行检查。
